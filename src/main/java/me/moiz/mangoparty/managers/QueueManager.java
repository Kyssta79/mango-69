...
public class QueueManager {
    ...
    public void leaveQueue(Player player) {
        ...
        if (entry != null) {
            ...
            if (queueList != null) {
                queueList.removeIf(e -> e.getPlayerUUID().equals(player.getUniqueId()));
    ...
    public void removePlayer(UUID playerId) {
        ...
        if (entry != null) {
            List<QueueEntry> queueList = queues.get(entry.getMode()).get(entry.getKitName());
            if (queueList != null) {
                queueList.removeIf(e -> e.getPlayerUUID().equals(playerId));
                if (queueList.isEmpty()) {
                    queues.get(entry.getMode()).remove(entry.getKitName());
                }
            }
    ...
    private void checkForMatch(String mode, String kitName) {
        ...
        if (queueList.size() >= requiredPlayers) {
            ...
            for (QueueEntry entry : matchEntries) {
                queueList.removeIf(e -> e.getPlayerUUID().equals(entry.getPlayerUUID()));
    ...
    private void startQueueMatch(String mode, String kitName, List<QueueEntry> entries) {
        ...
        if (arena == null) {
            ...
            for (QueueEntry entry : entries) {
                queues.get(mode).computeIfAbsent(kitName, k -> new ArrayList<>()).add(entry);
                playerQueues.put(entry.getPlayerUUID(), entry);
                Player player = entry.getPlayer();
                if (player != null) {
                    player.sendMessage("§cNo available arenas! Returned to queue.");
                }
            }
            return;
        }
        ...
        if (kit == null) {
            for (QueueEntry entry : entries) {
                Player player = entry.getPlayer();
                if (player != null) {
                    player.sendMessage("§cKit not found! Removed from queue.");
                }
            }
            return;
        }
